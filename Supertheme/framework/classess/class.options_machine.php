	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
                    $t_value = isset($ht_options[$value['id']]) && !empty($ht_options[$value['id']]) ? stripslashes(htmlspecialchars($ht_options[$value['id']])) : $value['std'];
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$select_ID.'" ' . selected($ht_options[$value['id']], $select_ID, false) . ' />'.$option.'</option>';
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
                   ...
                   // $ta_value = stripslashes($ht_options[$value['id']]);
                    $ta_value = isset($ht_options[$value['id']]) && !empty($ht_options[$value['id']]) ? stripslashes(htmlspecialchars($ht_options[$value['id']])) : stripslashes(htmlspecialchars($value['std']));
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($ht_options[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($ht_options[$value['id']])) {
						$ht_options[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($ht_options[$value['id']], 1, false) .' /><div class="clear"></div>';
				break;
				
				//multiple checkbox option
				case 'multicheck':
                    if(isset($ht_options[$value['id']])){
                        $multi_stored = $ht_options[$value['id']];
				...
				//colorpicker option
				case 'color':		
					$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$ht_options[$value['id']].'"></div></div>';
					$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $ht_options[$value['id']] .'" />';
				break;
				...
				case 'typography':
					...
					$typography_stored = isset($ht_options[$value['id']]) ? $ht_options[$value['id']] : $value['std'];
...
//typography option
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $ht_options[$value['id']];
				...
				case 'images':
					...
					$select_value = $ht_options[$value['id']];
				...
				case 'sorter':
					...
					$sortlists = isset($ht_options[$value['id']]) && !empty($ht_options[$value['id']]) ? $ht_options[$value['id']] : $value['std'];
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($ht_options[$value['id']]) && !empty($ht_options[$value['id']]) ? $ht_options[$value['id']] : '';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($ht_options)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
